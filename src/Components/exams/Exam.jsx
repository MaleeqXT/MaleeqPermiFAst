...
// ── Données ───────────────────────────────────────────────────────────────────
function toApiExamPayload(exam) {
  const time = exam.startTime;

  return {
    date_examen: exam.examDate || null,
    heure_passage: time ? `${String(time.hour).padStart(2, "0")}:${String(time.minute).padStart(2, "0")}` : null,
    comment: exam.comment || null,
    status: exam.status,
    result_permis: exam.resultPermis || null,
  };
}
  ...
  return (
    <div className="ex-toast">
      <div className="ex-toast-top">
        <IconInfo /> {toast.type === "error" ? "Erreur" : "Succès"}
        <button className="ex-toast-close" onClick={onClose}>✕</button>
      </div>
      <div className="ex-toast-bottom">{toast.message}</div>
    </div>
  );
}
...
export default function ExaminationList() {
  ...
  useEffect(() => {
    ...
    http.get("/admin/exams")
      .then(({ data }) => {
        if (active) setExams(Array.isArray(data?.data) ? data.data : []);
      })
      .catch(() => {
        if (active) setToast({ type: "error", message: "Impossible de charger les examens." });
      });
      ...
      <>
        ...
        <ExamDetail
          ...
          onSave={async (updated) => {
            try {
              const { data } = await http.put(`/admin/exams/${updated.id}`, toApiExamPayload(updated));
              const savedExam = data?.data;
              if (!savedExam) throw new Error("Réponse invalide du serveur");

              setExams(prev => prev.map(exam => exam.id === savedExam.id ? savedExam : exam));
              setEditTarget(null);
              setToast({ type: "success", message: "L'examen a été mis à jour avec succès." });
            } catch (error) {
              setToast({
                type: "error",
                message: error?.response?.data?.message || "Impossible d'enregistrer l'examen.",
              });
            }
          }}
        />
  ...
  const monitorOptions = [...new Set(exams.map(exam => exam.monitor).filter(Boolean))];
  const locationOptions = [...new Set(exams.map(exam => exam.location).filter(Boolean))];

  function saveComment(id, text) {
    http.put(`/admin/exams/${id}`, { comment: text })
      .then(({ data }) => {
        const savedExam = data?.data;
        if (!savedExam) throw new Error("Réponse invalide du serveur");
        setExams(prev => prev.map(exam => exam.id === savedExam.id ? savedExam : exam));
        setToast({ type: "success", message: "Le commentaire a été enregistré avec succès." });
      })
      .catch((error) => {
        setToast({
          type: "error",
          message: error?.response?.data?.message || "Impossible d'enregistrer le commentaire.",
        });
      });
  }
  ...
  return (
    <div className="cand-page">
      ...
      <div className="ex-filters-row">
        <FilterDropdown label="Moniteur" options={monitorOptions} value={monitorFilter} onChange={v => { setMonitorFilter(v); setPage(1); }} />
        <FilterDropdown label="Lieu"     options={locationOptions} value={locationFilter} onChange={v => { setLocationFilter(v); setPage(1); }} />
      </div>
