...
import ChatPageBoundary from "../chat/ChatPageBoundary.jsx";
import { createChatClient } from "../chat/chatClient.js";
import { chatError } from "../chat/chatData.js";
...
const MONITOR_SHORTCUTS = [
  { label: "Mon planning",  target: "accueil", icon: <CalendarIcon /> },
  { label: "Mes séances",   target: "sessions", icon: <CalendarIcon /> },
  { label: "Mon profil",    target: "settings", icon: <CalendarIcon /> },
];
...
// ─── Main Component ───────────────────────────────────────────────────────────
function MonitorChatPageContent({ monitorId = null, onNavigate }) {
  // A dashboard opened through Connecter is a staff session viewing a monitor.
  // Resolve its real chat user before fetching messages or subscribing to channels.
  ...
  useEffect(() => {
    ...
    client.get('/chat/session', { signal: controller.signal }).then(({ data }) => {
      if (!data?.user?.id) throw new Error('Invalid chat session response');
      if (!controller.signal.aborted) { setSession({ monitorId, user: data.user }); setError(''); }
    }).catch((err) => { if (!controller.signal.aborted) setError(chatError(err)); });
    return () => controller.abort();
  }, [client, monitorId, attempt]);
  if (error) return <p role="alert">{error} <button onClick={() => { setError(''); setAttempt((value) => value + 1); }}>Réessayer</button></p>;
  if (!session || session.monitorId !== monitorId) return <p role="status">Chargement de la messagerie…</p>;
  return <MonitorChatContent key={`${monitorId || 'self'}:${session.user.id}`} monitorId={monitorId} actorId={session.user.id} onNavigate={onNavigate} />;
}

export default function MonitorChatPage(props) {
  return <ChatPageBoundary><MonitorChatPageContent {...props} /></ChatPageBoundary>;
}

function MonitorChatContent({ monitorId, actorId, onNavigate }) {
  const chat = useChat({ monitorId, actorId });
  ...
  return (
    <div className={`nschat-root nschat-root--monitor${mobileChatOpen ? " nschat-mobile-show-chat" : ""}`} style={{ height: "100%", background: "#f5f6f8" }}>
      {chat.newChatOpen && <NewChatDialog client={chat.client} onClose={() => chat.setNewChatOpen(false)} onStart={startConversation} />}
      <section className="nschat-workspace" style={{ height: "100%", marginTop: 0 }} aria-label="Messagerie Moniteur">
        ...
        <aside className={`nschat-info-column${mobileDetailsOpen ? " is-mobile-open" : ""}`}>
          <section className="nschat-card nschat-about-card">
            <h2>À propos</h2>
            <div className="nschat-about-person">
              <Avatar conversation={selectedConversation} size="large" />
              <div><strong>{selectedConversation.name}</strong><span>{selectedConversation.role || "Conversation de groupe"}</span></div>
            </div>
          ...
          <section className="nschat-card nschat-shortcuts-card">
            <h2>Raccourcis</h2>
            <div>
              {MONITOR_SHORTCUTS.map((s) => (
                <button key={s.label} type="button" onClick={() => onNavigate?.(s.target)}>
                  <span>{s.icon}</span>{s.label}<ChevronRightIcon />
                </button>
              ))}
            </div>
          </section>
          ...
          <section className="nschat-hours-card">
            <header><ClockIcon /><h2>Disponibilités</h2></header>
            <div>
              <p><strong>Lundi - Vendredi</strong><span>08h00 - 18h00</span></p>
              <p><strong>Samedi</strong><span>09h00 - 13h00</span></p>
            </div>
            <button type="button" onClick={() => onNavigate?.('settings')}>Gérer mes disponibilités</button>
          </section>
        </aside>
      </section>
    </div>
