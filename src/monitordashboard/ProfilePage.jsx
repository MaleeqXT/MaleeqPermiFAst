...
export default function ProfilePage({ onBack, monitorId: selectedMonitorId = null }) {
  ...
  const displayName = `${form.first_name} ${form.last_name}`.trim() || resolveDisplayName(profile, resolveDisplayName(currentUser, "—"));
  const avatarSrc = media || (profile ? resolveAvatarSrc(profile) : null) || resolveAvatarSrc(currentUser);
