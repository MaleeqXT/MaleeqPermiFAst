...
import MonitorChatPage from "./MonitorChatPage.jsx";
...
import { profileImageUrl } from "../new-student-dashboard/StudentHeader.jsx";

const resolveDisplayName = (user) => {
  if (!user) return "Moniteur";
  const name = [user.first_name, user.last_name].filter(Boolean).join(" ").trim() || user.name;
  return name || "Moniteur";
};

const resolveMonitorUser = (target) => {
  if (!target) return null;
  if (target.user && typeof target.user === "object") {
    return { ...target.user, monitor: target.monitor || target };
  }
  if (target.data && typeof target.data === "object") {
    return resolveMonitorUser(target.data);
  }
  return target;
};
...
export default function MonitorDashboard({ onClose }) {
  ...
  const monitorAvatar = profileImageUrl(activeMonitorUser);
  const [imgFailed, setImgFailed] = useState(false);
      ...
      <aside className={`md-sidebar ${collapsed ? "md-sidebar--collapsed" : ""}`}>
        <div className="md-sidebar-top">
          <div className="md-sidebar-brand">
            {collapsed ? (
              <img src="https://passpermisfacile.fr/assets/logo.svg" alt="PassPermis" className="md-sidebar-logo" />
            ) : (
              <img src="http://127.0.0.1:8000/assets/logo-permis-facile.webp" alt="PassPermis" className="md-sidebar-logo-full" />
            )}
          </div>
        ...
        <nav className="md-nav">
          {NAV_ITEMS.map((item) => (
            ...
            >
              ...
              {item.id === 'messages' && <ChatUnreadBadge monitorId={viewedMonitorId} collapsed={collapsed} />}
            </button>
          ))}
        </nav>
      </aside>
        ...
        <main className="md-content">
          ...
          {activeTab === "messages" && (
            <div style={{ height: "100%", minHeight: 0, display: "flex", flexDirection: "column" }}>
              <MonitorChatPage key={viewedMonitorId || currentUser?.id} monitorId={viewedMonitorId} onNavigate={setActiveTab} />
            </div>
          )}
          {activeTab === "settings" && (
            <TabSettings
